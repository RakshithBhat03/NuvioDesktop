#!/usr/bin/env python3
"""Verify subtitle/audio switching and HTTP byte counts with bundled macOS mpv.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="nuvio-subtitle-ram-") as directory:
        ...
        subprocess.run([
            ...
            "sine=frequency=440:duration=40", "-f", "lavfi", "-i",
            "sine=frequency=880:duration=40", "-i", str(root / "1.srt"),
            "-i", str(root / "2.srt"), "-map", "0", "-map", "1", "-map", "2", "-map", "3", "-map", "4",
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                if self.headers.get("X-Nuvio-Test") != "tracks":
                    with lock:
                        counters["denied"] += 1
                    self.send_error(403)
                    return
        ...
        render_executable = root / "render-test"
        subprocess.run([
            "clang++", "-arch", arch, "-std=c++17", "-fobjc-arc",
            "-Wno-deprecated-declarations",
            "-framework", "Foundation", "-framework", "OpenGL",
            str(tests / "subtitle_ram_render_test.mm"), "-o", str(render_executable),
            f"-I{tests.parent / 'include'}", str(runtime / "libmpv.2.dylib"),
            f"-Wl,-rpath,{runtime}",
        ], check=True)
        ...
        try:
            modes = ("baseline", "cached", "capped", "audio-baseline", "audio-cached")
            cases = [*itertools.product(("test.mkv", "test.mp4"), modes),
                     ("streaming.mkv", "streaming"),
                     ("streaming.mkv", "audio-streaming")]
            if os.environ.get("NUVIO_TEST_RENDER_ONLY") == "1":
                cases = []
            for filename, mode in cases:
                ...
                try:
                    ...
                    if "baseline" not in mode:
                        assert "streaming" in mode or extra_bytes == 0, "Cached switching/seeking must not download media again"
                    ...
                    if process.poll() is None:
                        process.kill()
                        process.wait()
            subprocess.run([str(render_executable), str(root / "test.mkv")],
