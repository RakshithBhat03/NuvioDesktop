...
static int64_t cacheBytes(mpv_handle *mpv, const char *key) {
    mpv_node state{};
    ...
    for (int i = 0; i < state.u.list->num; i++) {
        if (std::string(state.u.list->keys[i]) == key)
            bytes = state.u.list->values[i].u.int64;
    }
    mpv_free_node_contents(&state);
    check(bytes >= 0, "Track cache accounting must be exposed");
    return bytes;
}
...
int main(int argc, char **argv) {
    @autoreleasepool {
        ...
        bool audio = mode.rfind("audio-", 0) == 0;
        bool cached = mode.find("baseline") == std::string::npos;
        bool streaming = mode.find("streaming") != std::string::npos;
        bool capped = mode == "capped";
        mpv_handle *mpv = mpv_create();
        check(mpv != nullptr, "mpv_create");
        for (auto pair : {std::pair{"config", "no"}, {"vo", "null"}, {"ao", "null"},
             {"terminal", "no"}, {"idle", "yes"}, {"keep-open", "yes"}, {"sid", "1"}, {"aid", "1"},
             {"cache", "auto"}, {"cache-secs", "120"}, {"demuxer-max-bytes", "150MiB"},
             {"http-header-fields", "X-Nuvio-Test: tracks"}}) {
        ...
        check(mpv_set_option_string(mpv, "demuxer-audio-cache-bytes", cached ? "64MiB" : "0") >= 0,
              "Patched mpv audio cache option");
        if (streaming) {
        ...
        waitFor(mpv, 6);
        if (!streaming) {
            // Reaching the playback timestamp does not mean the background
            // demuxer has finished filling. Wait for the condition this test
            // actually needs instead of racing the reader on faster machines.
            for (int i = 0; i < 800; i++) {
                if (flag(mpv, "demuxer-cache-idle") &&
                    number(mpv, "demuxer-cache-duration") > 20)
                    break;
        ...
        const char *cacheKey = audio ? "audio-cache-bytes" : "subtitle-cache-bytes";
        double retained = cacheBytes(mpv, cacheKey);
        double limit = capped ? 1024 : (audio ? 64 : 32) * 1024 * 1024;
        check(retained <= limit, "Track RAM cap");
        check(!cached || retained > 0, "Cache must contain track packets");
        std::puts("READY");
        std::fflush(stdout);
        ...
        check(mpv_set_property_string(mpv, audio ? "aid" : "sid", "2") >= 0,
              audio ? "Select built-in audio 2" : "Select built-in subtitle 2");
        ...
        std::printf("RESULT kind=%s elapsed=%.3f progress=%.3f loading_samples=%d cache_bytes=%.0f\n",
                    audio ? "audio" : "subtitle", elapsed, progress, loadingSamples, retained);
        std::fflush(stdout);
        if (cached) {
            if (audio) {
                check(number(mpv, "aid") == 2, "Newly selected audio must remain active");
            } else {
            ...
            check(progress > elapsed - 0.25, "Cached track switching must avoid the playback stall");
            check(loadingSamples < 20, "Cached switching must not trigger sustained buffering");
            check(number(mpv, "track-list/count") == 5, "No extra reader or duplicate tracks");
            if (!streaming && audio) {
                check(mpv_set_property_string(mpv, "pause", "yes") >= 0, "Pause");
                check(mpv_set_property_string(mpv, "aid", "1") >= 0, "Switch audio back");
                check(flag(mpv, "pause"), "Preserve an explicit pause");
                check(number(mpv, "aid") == 1, "Original audio must remain selectable");
            }
            if (!streaming && !audio) {
                check(mpv_set_property_string(mpv, "pause", "yes") >= 0, "Pause");
        ...
        for (int i = 0; i < 100 &&
             (cacheBytes(mpv, "subtitle-cache-bytes") != 0 ||
              cacheBytes(mpv, "audio-cache-bytes") != 0); i++)
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        check(cacheBytes(mpv, "subtitle-cache-bytes") == 0 &&
              cacheBytes(mpv, "audio-cache-bytes") == 0,
              "Release track caches on stream close");
