...
int main(int argc, char **argv) {
    ...
    mpv_set_option_string(player, "aid", "1");
    require(mpv_set_option_string(player, "demuxer-subtitle-cache-bytes", "32MiB") >= 0, "RAM cache option");
    require(mpv_set_option_string(player, "demuxer-audio-cache-bytes", "64MiB") >= 0, "Audio RAM cache option");
    ...
    for (int i = 0; i < 1200; i++) {
        ...
        if (!audioSwitched && position > 2.5) {
            mpv_set_property_string(player, "aid", "2");
            audioSwitched = true;
        }
    ...
    require(switched && audioSwitched && frames > 30,
            "Playback must render frames across subtitle and audio switches");
